...
$sql_year_log = "SELECT MONTH(`date`) as cmonth,Year(`date`) as cyear, avg(time_duration) as avgs  FROM `totalcall` group by month(`date`),Year(`date`) 
...
while ($row2 = mysql_fetch_assoc($query_year)) {
    ...
    if($callyear == $get_year){
        $arr_average_year[$count] = (float)$avg;
...
echo json_encode(array("total_call_year"=>$temp_arr_year));
mysql_close();
?>